...
describe('DiceRoller', () => {
  ...
  it('rolls simple 1d4', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('1d4'))).toBe(2)
  })
  ...
  it('rolls all the standard dice', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('1d6'))).toBe(1)
    expect(diceRoller.roll(new DiceRollSpec('1d8'))).toBe(2)
    expect(diceRoller.roll(new DiceRollSpec('1d10'))).toBe(3)
    expect(diceRoller.roll(new DiceRollSpec('1d12'))).toBe(10)
    expect(diceRoller.roll(new DiceRollSpec('1d20'))).toBe(20)
    expect(diceRoller.roll(new DiceRollSpec('1d100'))).toBe(1)
  })
  it('works well with multiple rolls of the same die', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('1d6'))).toBe(1)
    expect(diceRoller.roll(new DiceRollSpec('1d6'))).toBe(4)
  })
  it('rolls 2d4', () => {
    const a = 2
    const b = 4
    ...
    expect(diceRoller.roll(new DiceRollSpec('2d4'))).toBe(a + b)
  })
  it('can roll any (x)d(y)', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('4d12'))).toBe(3 + 5 + 1 + 12)
  })

  it('can roll 1d4+1', () => {
    random.setNextForRange(2, 1, 4)

    expect(diceRoller.roll(new DiceRollSpec('1d4+1'))).toBe(2 + 1)
  })
  ...
  it('can roll (x)d(y)+z', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('2d100+23'))).toBe(13 + 44 + 23)
  })
  ...
  it('can roll 2d4 + 1d8 + 3', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('2d4 + 1d8 + 3'))).toBe(1 + 4 + 3 + 3)
  })
  ...
  it('rolls result that is always non-negative', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('1d4-300'))).toBe(0)
  })
  ...
  it('handles very complex cases well', () => {
    ...
    expect(diceRoller.roll(new DiceRollSpec('2d4 - 1d8 + 3 + 3d4 + 4 - 1'))).toBe(1 + 4 - 3 + 3 + 1 + 2 + 3 + 4 - 1)
  })
